...
export async function POST(req: Request) {
  try {
    ...
    if (hasApproveAll) {
      ...
      const hit = (privileges || []).find((p: any) => p.privilege === "APPROVE_ONBOARD_ALL" && p.execute);
      ...
      for (const m of activeMemberships) {
        ...
        if (paramsAllowsYear(params, profile.graduation_year)) {
          allowed = true;
      ...
      if (!allowed) {
        ...
        const approverYear = approverProfile?.graduation_year ?? null;
        if (approverYear !== null && approverYear === profile.graduation_year) {
          allowed = true;
          // membership type unknown for audit in fallback; leave membershipTypeUsed null
        }
      }
    }
